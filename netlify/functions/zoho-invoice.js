...
async function findContact(token, email) {
  const res = await fetch(`${ZOHO_API_BASE}/contacts?organization_id=${ORG_ID}&contact_type=customer&search_text=${encodeURIComponent(email)}`, {
    headers: { Authorization: `Zoho-oauthtoken ${token}` },
  });
  const data = await res.json();
  if (data.contacts && data.contacts.length > 0) {
    // Match on email to be precise
...
async function createContact(token, { name, email, phone }) {
  ...
  const body = {
    ...
    customer_sub_type: 'individual',   // individual, not business
    email,
    tax_exemption_code: 'GST FREE',    // GST free
    tax_authority_name: 'GST FREE',
    contact_persons: [{
